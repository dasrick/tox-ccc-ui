...
describe('Components:Security:Service:SecurityService', function () {
  ...
  var SecurityServiceInstance, $httpBackend, UserService, AlertService, locals;
  ...
  it('should login an user and set the jwt token', function () {
    $httpBackend.when('POST', baseUrl + '/api/security/login').respond({token: tokenValid});
  ...
  it('should try to login an user and return an unknown error', function () {
    ...
    $httpBackend.when('POST', baseUrl + '/api/security/login').respond(status);
    $httpBackend.expectPOST('https://ccc.mi24.dev/api/security/login', 'data');
    SecurityServiceInstance.login('data');
    $httpBackend.flush();
  ...
  it('should try to login an user and return error 401', function () {
    var status = 401;
    var msg = 'security.alert.login.unauthorized';
    $httpBackend.when('POST', baseUrl + '/api/security/login').respond(status);
    $httpBackend.expectPOST('https://ccc.mi24.dev/api/security/login', 'data');
    SecurityServiceInstance.login('data');
    $httpBackend.flush();
  ...
  it('should try to login an user and return error 401 and header "locked"', function () {
    ...
    var msg = 'security.alert.login.user_locked';
    $httpBackend.when('POST', baseUrl + '/api/security/login').respond(status, data, headers);
    $httpBackend.expectPOST('https://ccc.mi24.dev/api/security/login', 'data');
    SecurityServiceInstance.login('data');
    $httpBackend.flush();
    expect(AlertService.add).toHaveBeenCalledWith('danger', msg, 10000);
